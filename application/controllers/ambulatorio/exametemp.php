...
//        die;
        ...
        @$nome = str_replace(' ', '|||', $cpf_array[0]->nome);
        $parceiro = $this->parceiro->listarparceiroendereco($parceiro_id);
        @$endereco = $parceiro[0]->endereco_ip;
...
//        die;
        ...
        if (($dias_parcela - $dias_atendimento) >= $carencia) {
            ...
            if (@$cpf_array[0]->nascimento != '') {
//                var_dump($_POST['nascimento']); die;
                $nascimento = date("Y-m-d", strtotime(str_replace('/', '-', @$cpf_array[0]->nascimento)));
            } else {
                $nascimento = '';
            }

            $celular = str_replace(' ', '', @$cpf_array[0]->celular);
            $telefone = str_replace(' ', '', @$cpf_array[0]->telefone);
